...
export const spawnSpriteForTask = (config: SpawnSpriteConfig) =>
  Effect.gen(function* () {
    ...
    if (Option.isSome(opencodeAuth)) {
      ...
      yield* sprites
        .execCommand(
          ...
          { stdin: opencodeAuth.value, maxRunAfterDisconnect: SETUP_MAX_RUN_DISCONNECT }
        )
        .pipe(
          Effect.mapError(
            error =>
    ...
    yield* sprites
      .execCommand(spriteName, ['bash', '-c', `cat > /tmp/abraxas-run.sh`], {
        stdin: script,
        maxRunAfterDisconnect: SETUP_MAX_RUN_DISCONNECT
      })
      .pipe(
    ...
    // Make it executable
    yield* sprites
      .execCommand(spriteName, ['chmod', '+x', '/tmp/abraxas-run.sh'], {
        maxRunAfterDisconnect: SETUP_MAX_RUN_DISCONNECT
      })
      .pipe(
        Effect.mapError(
    ...
    yield* sprites
      .execCommand(
        spriteName,
        ['bash', '-c', 'bash /tmp/abraxas-run.sh > /tmp/abraxas.log 2>&1'],
        { maxRunAfterDisconnect: SETUP_MAX_RUN_DISCONNECT }
      )
      .pipe(
        Effect.mapError(
